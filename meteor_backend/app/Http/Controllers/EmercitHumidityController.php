...
class EmercitHumidityController extends Controller
{
    /**
     * Получение новых данных по влажности со станций эмерсит, сопоставление идентификаторов внешней и общей базы
     ...
     * @return string
     */
    static public function getNewHumidityDataEmercit($station_id, $station_id_emercit, $dataType)
    {
        ...
        if (!isset($data[0]['created_at'])) {
            EmercitHumidity::where('station_id', $station_id_emercit)->chunk(10000, function ($chunks) use ($station_id, $dataType, $station_id_emercit){
            ...
            EmercitHumidity::where('station_id', $station_id_emercit)->where('time_utc','>',$data[0]['created_at'])->chunk(10000, function ($chunks) use ($station_id, $dataType, $station_id_emercit) {
                foreach ($chunks as $chunk) {
                    $ar[] = ['device' => $station_id, 'dataType' => $dataType, 'value' => $chunk->value, 'created_at' => $chunk->time_utc, 'updated_at' => Carbon::now()];
                }
                MeteoHistory::insert($ar);
